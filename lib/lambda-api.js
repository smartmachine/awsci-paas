...
class LambdaApi extends Construct {
  ...
  addAuthorizer(userpoolArn) {
    ...
    const child = this.method.node.defaultChild;
    LambdaApi.override(child, 'AuthorizationType', 'COGNITO_USER_POOLS');
    LambdaApi.override(child, 'AuthorizerId', { Ref: authorizer.logicalId });
  }
   ...
   * @param {object}       value
   */
  static override(method, path, value) {
