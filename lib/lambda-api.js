...
class LambdaApi extends Construct {
  ...
  constructor(scope, id, options) {
    ...
    const method = res.addMethod(options.httpMethod, integration, methodOptions);

    const authorizer = new CfnAuthorizer(this, 'Authorizer', {
      name:      "cognitoAuthorizer",
      restApiId: options.api.restApiId,
      type: 'COGNITO_USER_POOLS',
      identitySource: 'method.request.header.Authorization',
      providerArns: ["somearn"],
    });

    const child = method.node.defaultChild;
